...
class CornersProblem(search.SearchProblem):
  ...
  def __init__(self, startingGameState):
    ...
    self.goal = (self.startingPosition, (True, True, True, True))
  ...
  def getSuccessors(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      nextx, nexty = int(x + dx), int(y + dy)
      if not self.walls[nextx][nexty]:
        cornersvisited = [state[1][0], state[1][1], state[1][2], state[1][3]]
        ...
        if (nextx, nexty) in self.corners:
          ...
          cornersvisited[idx] = True


        nextState = ((nextx, nexty), (cornersvisited[0], cornersvisited[1], cornersvisited[2], cornersvisited[3]))
        #cost = self.costFn(nextState)
...
def cornersHeuristic(state, problem):
  ...
  for corner_status in corners_status:
    if not corner_status:
      corner = corners[idx]
      result = abs(pos[0] - corner[0]) + abs(pos[1] - corner[1])
      break
