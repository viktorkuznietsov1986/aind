...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        s_literals = self.s_levels[level]
        self.a_levels.append(set())

        for a in self.all_actions:
            node_a = PgNode_a(a)
            if node_a.prenodes.intersection(s_literals):
                if not self.a_levels[level].__contains__(node_a):
                    self.a_levels[level].add(node_a)

                    for node_s in s_literals:
    ...
    def add_literal_level(self, level):
        ...
        for anode in actions:
            effnodes = anode.effnodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return len(set(node_a1.action.effect_add).intersection(set(node_a2.action.effect_rem))) > 0 \
                or len(set(node_a2.action.effect_add).intersection(set(node_a1.action.effect_rem))) > 0
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return len(set(node_a1.action.effect_rem).intersection(set(node_a2.action.precond_pos))) > 0 \
               or len(set(node_a2.action.effect_rem).intersection(set(node_a1.action.precond_pos))) > 0
    ...
    def h_levelsum(self) -> int:
        ...
        levels_size = len(self.s_levels)

        for i in range(0, levels_size):
            if len(goals) == 0:
                break

            for fluent in self.s_levels[i]:

                if fluent.is_pos:
                    goal = fluent.symbol
                    if goals.__contains__(goal):
                        goals.remove(goal)
                        level_sum += i
                        break
